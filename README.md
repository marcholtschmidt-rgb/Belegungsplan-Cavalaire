...
- `manifest-v2.json` – Konfiguration für die "Installieren als App"-Funktion
- `icon-192-v2.png` / `icon-512-v2.png` – App-Icon (Seestern-Logo von Cavalaire-sur-Mer)

> **Hinweis zur Versionierung:** Die Dateien heißen bewusst `-v2`, weil iOS Safari `manifest.json` und die darin verlinkten Icons extrem hartnäckig cacht – ein reines Ersetzen unter altem Dateinamen zeigt oft tagelang noch das alte Icon im "Zum Home-Bildschirm hinzufügen"-Dialog. Bei einer zukünftigen Icon-Änderung am besten wieder auf einen neuen Dateinamen (`-v3` usw.) wechseln und `index.html` entsprechend anpassen. Die alten Dateien (`manifest.json`, `icon-192.png`, `icon-512.png`) können bei GitHub gelöscht werden, sind aber nicht mehr referenziert.
